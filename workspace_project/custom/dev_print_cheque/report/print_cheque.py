...
class print_check(models.AbstractModel): 
    ...
    def amount_word(self, obj):
        ...
        if float(amt_lst[1]) > 0:
            if obj.cheque_formate_id.decimal_format_type == 'default':
                    lst.append(' and '+amt_lst[1]+' / '+str(100))
            else:
                fraction_part = int(math.ceil(obj.amount * 100) % 100)
                float_str = num2words(fraction_part)
                lst.append(' and ' + float_str + ' ' + obj.cheque_formate_id.decimal_value)
        if obj.cheque_formate_id.print_only:
            lst.append('only')
        lst_len = len(lst)
        first_line = ''
        second_line = ''
